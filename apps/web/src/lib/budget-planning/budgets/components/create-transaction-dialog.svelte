<script lang="ts">
  ...
  type CreateTransactionSchema = ReturnType<
    typeof createCreateTransactionSchema
  >
  ...
  type Props = {
    ...
    data: SuperValidated<Infer<CreateTransactionSchema>>
    error: CreateTransactionError | undefined
  ...
  $effect(() => {
    ...
    $formData.budgetCategoryId = budgetCategoryId
    $formData.name = ''
    $formData.amount = 0
...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
  <Dialog.Content class="sm:max-w-md">
    <Dialog.Header>
      ...
      <Dialog.Description>
        {categoryName
          ? m.budget_detail_transaction_create_description({ categoryName })
          : ''}
      </Dialog.Description>
    </Dialog.Header>
    ...
    >
      ...
      <Dialog.Footer>
        ...
        <Form.Button disabled={$submitting}>
          {$submitting
            ? m.budget_detail_transaction_submitting()
            : m.budget_detail_transaction_submit()}
        </Form.Button>
      </Dialog.Footer>
    </form>
  </Dialog.Content>
