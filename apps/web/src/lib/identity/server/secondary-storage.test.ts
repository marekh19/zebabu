...
const redisSecondaryStorage = createRedisSecondaryStorage(
  'zebabu:test:better-auth',
)
...
describe('set', () => {
  it('applies the TTL in the same command as the write', async () => {
    ...
    expect(mockRedis.set).toHaveBeenCalledWith(
      'zebabu:test:better-auth:session:1',
      'payload',
      'EX',
      60,
    )
...
describe('increment', () => {
  it('evaluates the counter script with one key and returns the new count', async () => {
    ...
    expect(key).toBe('zebabu:test:better-auth:rate:ip')
    expect(ttl).toBe(60)
...
describe('getAndDelete', () => {
  it('uses a single atomic GETDEL', async () => {
    ...
    expect(mockRedis.getdel).toHaveBeenCalledWith(
      'zebabu:test:better-auth:verify:1',
    )
  })
